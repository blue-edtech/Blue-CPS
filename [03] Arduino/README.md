...
Vamos iniciar nosso projeto com uma _protoboard_ com `10 LEDs`, seus resistores e um Arduíno para realizar o controle.

Ligaremos o **GNT** na fileira superior da placa e os negativos de cada LED à ela.

Cada LED será ligado à uma porta digital no Arduíno:

![Aula03_Figura40](imagens/Aula03_Figura40.png)

Vamos iniciar nossa programação criando as variáveis para cada LED armazenando suas portas:
...
Criando nosso `void setup()` com o `pinMode()` para cada LED:
...
No nosso `void loop()` nós iremos piscar **um LED por vez da esquerda para a direita e da direita para a esquerda** , para 10 LEDs, e opa!! precisaremos de funções para isso, vamos à elas antes:
...
Onde `tempo` é uma variável que precisamos definir e que determinará o tempo em milissegundos que cada LED irá piscar e note que ela receberá um nome diferente pois ela será o parâmetro que usaremos em nosso `void loop`():

```c++
int time = 100;
```

Usando a variável `time` globalmente, nos dá a maleabilidade de determinar um outro tempo para qualquer LED que desejarmos.

Ao final, o nosso código deverá estar assim:
...
Até aqui, nós temos um código com 130 linhas. Vamos deixá-lo menor?

Vamos voltar à página inicial do _Thinkercat_, duplicar este projeto e no novo, otimizaremos o código para:

```c++
void setup()
{
  for(int porta = 13; porta >=4;porta--)
  {
...
Nosso desafio para você com este código otimizado é:

- Deixe em nossos comentários no YouTube o que está acontecendo no código em cada linha.
